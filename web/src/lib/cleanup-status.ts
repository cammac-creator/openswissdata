...
export function renderCleanupStatus(proof: CleanupProof | null | undefined, now = Date.now(), schedule?: Schedule): string {
  ...
  const title = schedule?.active === false ? 'Programmation à vérifier' : laterFailure ? 'Dernière exécution en échec' : !proof ? 'Nettoyage à confirmer' : !proof.ok ? 'Nettoyage incomplet' : !recent ? 'Nettoyage à revérifier' : 'Nettoyage périodique vérifié';
  ...
  return `<section class="panel" style="margin-bottom:24px"><div class="panel-heading"><div><h2>${title}</h2><p>${date ? `Dernier témoin conservé : ${date} · heure suisse.` : 'Aucun témoin complet disponible.'}</p></div><span class="pill ${healthy ? 'green' : 'amber'}">${healthy ? 'Vérifié' : 'À vérifier'}</span></div><p class="fine">Prévu toutes les six heures. Les copies techniques sont purgées même sans visite du bureau.</p>${proof ? `<details style="margin-top:16px"><summary>Voir les catégories contrôlées</summary>${rows}</details>` : ''}<p class="source-note">Les commandes, preuves contractuelles, droits d’accès, notes et courriers originaux ne sont pas supprimés par ce nettoyage. Les sauvegardes ont leur propre cycle de conservation.</p></section>`;
}
