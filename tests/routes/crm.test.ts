...
describe('Bureau privé et suivi client',()=>{
 ...
 it('présente une audience cohérente, ses jours absents et la limite de conservation',async()=>{
  ...
  expect(a.daily.find((x:{day:string})=>x.day==='2026-09-24')).toEqual({day:'2026-09-24',views:1,visitors:1,partial:true,incomplete:false,unidentified_views:0});
  expect(a.daily.find((x:{day:string})=>x.day==='2026-09-25')).toEqual({day:'2026-09-25',views:3,visitors:2,partial:false,incomplete:false,unidentified_views:0});
  expect(a.daily.at(-1)).toEqual({day:'2026-09-26',views:0,visitors:0,partial:true,incomplete:false,unidentified_views:0});
  expect(a.daily[0]).toEqual({day:'2026-09-20',views:null,visitors:null,partial:false,incomplete:false,unidentified_views:null});
 ...
 it('exclut les pages déclarées de l’audience et expose l’origine historique sans la réécrire',async()=>{
  const now=Date.now(),db=getDb();const event=db.prepare("INSERT INTO events(kind,name,origin,visitor_hash,ua_class,ts) VALUES('custom','page_view',?,?,'desktop',?)");
  for(const origin of ['server','legacy','client'])event.run(origin,origin,now);
  const a=await(await createApp().request('/api/admin/crm/audience?days=7',{headers})).json();
  expect(a.web).toEqual({pageviews:2,visitor_days:2});expect(a.page_origins).toEqual([{label:'legacy',count:1},{label:'server',count:1}]);expect(a.split).toEqual([{label:'desktop',count:2}]);
  expect(db.prepare("SELECT origin FROM events WHERE visitor_hash='legacy'").get()).toEqual({origin:'legacy'});
 });
