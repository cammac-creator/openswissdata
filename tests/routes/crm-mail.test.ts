...
describe('Périmètre de la messagerie personnelle',()=>{
 ...
 afterEach(()=>{closeDb();rmSync(temp,{recursive:true,force:true});delete process.env.DATABASE_PATH;delete process.env.OSD_BACKUP_KEY;delete process.env.CRM_PROJECT_MAILBOX;vi.unstubAllGlobals();clearCrmCache()});
 it('lit le HTML quand Resend fournit un texte vide et conserve la langue déclarée du mail',async()=>{process.env.RESEND_API_KEY='fictif';vi.stubGlobal('fetch',vi.fn().mockResolvedValue(Response.json({from:'contact@openswissdata.com',to:['buyer@example.test'],subject:'Your file',created_at:new Date().toISOString(),last_event:'delivered',text:'',html:'<html lang="en"><body><p>Hello, your updated file is available.</p></body></html>'})));const r=await app.request('/mail/resend/11111111-1111-4111-8111-111111111111');expect(r.status).toBe(200);const body=await r.json();expect(body.text).toContain('Hello, your updated file is available.');expect(body.language).toMatchObject({code:'en',source:'template'});delete process.env.RESEND_API_KEY});
 it('masque les liens de livraison actuels et historiques dans la lecture du message',async()=>{
  process.env.RESEND_API_KEY='fictif';const secret='A'.repeat(43);
  vi.stubGlobal('fetch',vi.fn().mockResolvedValue(Response.json({from:'contact@openswissdata.com',to:['buyer@example.test'],subject:'Fichier',created_at:new Date().toISOString(),last_event:'delivered',text:`Lien https://www.openswissdata.com/api/delivery/${secret}?lang=fr et ancien https://www.openswissdata.com/api/download/${secret}`})));
  const response=await app.request('/mail/resend/11111111-1111-4111-8111-111111111111');
  expect(response.status).toBe(200);const body=await response.json();expect(JSON.stringify(body)).not.toContain(secret);expect(body.text).toContain('[lien de téléchargement personnel masqué]');delete process.env.RESEND_API_KEY;
 });
 it('masque également un lien relatif issu de la conversion HTML',async()=>{
  process.env.RESEND_API_KEY='fictif';const secret='B'.repeat(43);
  vi.stubGlobal('fetch',vi.fn().mockResolvedValue(Response.json({from:'contact@openswissdata.com',to:['buyer@example.test'],subject:'Fichier',created_at:new Date().toISOString(),last_event:'delivered',text:'',html:`<p><a href="/api/delivery/${secret}?lang=fr">Fichier</a></p>`})));
  const response=await app.request('/mail/resend/11111111-1111-4111-8111-111111111111');
  expect(response.status).toBe(200);expect(JSON.stringify(await response.json())).not.toContain(secret);delete process.env.RESEND_API_KEY;
 });
